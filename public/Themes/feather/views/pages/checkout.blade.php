		 ...
		 @foreach ($cart as $key => $item)
			<tr class="cart_item">
				...
				<td class="product-price bf" data-title="Price">
					<span class="amount"><bdi><span>{!! Currency::symbol($user->gateway['currency'] ?? '') !!}</span> {{ nf($item->price * $item->quantity) }}</bdi></span>
				</td>
				...
				<table class="table cart-totals">
					<tbody>
						...
						<tr>
							...
							<td class="total-in-cart" data-total="{{ \App\Cart::total($uid) }}">{!! Currency::symbol($user->gateway['currency'] ?? '') !!} <span>{{ nf(\App\Cart::total($uid)) }}</span></td>
						</tr>
					</tbody>
				</table>
