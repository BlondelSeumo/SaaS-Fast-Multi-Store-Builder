   ...
   # devine general settings
    ...
    public function edit_post_page(Request $request){
        ...
        if (!empty($request->image)) {
            ...
            if (!empty($page->image)) {
                if(file_exists($_SERVER['DOCUMENT_ROOT'] . '/media/pages/' . $page->image)){
                    unlink($_SERVER['DOCUMENT_ROOT'] . '/media/pages/' . $page->image); 
                }
            }
            $request->image->move(public_path('media/pages'), $imageName);
    ...
    public function post_settings(Request $request, Domains $domains) {
        ...
        if (!empty($request->logo)) {
            ...
            if (!empty($this->settings->logo)) {
                if(file_exists(media_path('logo/' . $this->settings->logo))){
                    unlink(media_path('logo/' . $this->settings->logo)); 
                }
            }
            $request->logo->move(media_path('logo'), $imageName);
        ...
        if (!empty($request->favicon)) {
            ...
            if (!empty($this->settings->favicon)) {
                if(file_exists(media_path('favicon/' . $this->settings->favicon))){
                    unlink(media_path('favicon/' . $this->settings->favicon)); 
                }
            }
            $request->favicon->move(media_path('favicon'), $imageName);
            $values = array('value' => $imageName);
